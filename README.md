...
| `토큰로그` | **careerTest 공용 탭에 미러링** | careerTest 형식 8열 그대로 (검사종류 = `진로상담:1차`) |

미러링 덕분에 기존 careerTest 합계·차트에 진로상담 사용량이 함께 잡힌다.

> ⚠ **공용 탭의 열 형식을 바꾸지 말 것.** careerTest 쪽이 깨진다.
...
추적할 수 있어야 하기 때문이다. 다만 토큰이 0이므로 공용 탭은 어지럽히지 않는다.
