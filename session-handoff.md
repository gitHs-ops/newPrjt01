...
- 🔴 **배포본이 낡았다 — 배포=1.7.1 / 저장소=1.7.2.** `.\init.ps1 -Live` 로 확인(2026-09-01).
  빠진 것은 v1.7.2 의 **로그 정정**(이어쓰기로 `tools` 를 비우는 바람에 시트의 `web_tools`
  가 `off` 로 뒤집히던 문제 + 이어쓰기 횟수 기록)이다. 분석 품질에는 영향이 없고
  **시트 로그 열이 틀리게 적힌다.** 이 상태에서는 결과 화면에도 "프록시가 낡았습니다"
  경고가 뜬다. **배포 관리 → 편집 → 새 버전**으로 재배포하면 해소된다
